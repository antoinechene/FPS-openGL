...
ID::Control::Control() : _window(NULL),
			 _parent(NULL),
			 _cbClick(NULL),
			 _cbDoubleClick(NULL),
			 _cbClickDown(NULL),
...
int	ID::Control::DelChild(ID::Drawable* drawable)
{
	...
	while (it != end)
	{
		if (*it == drawable)
		{
			if (w != NULL)
			{
				if (w->GetDrawableFocused() == drawable)
					w->SetDrawableFocused(NULL);
				if (w->GetDrawableEnter() == drawable)
					w->SetDrawableEnter(NULL);
			}
			this->_childrenDrawable.erase(it);
			drawable->GetParent()->SetInvalidate(true);
			drawable->SetParent(NULL);
			drawable->SetWindow(NULL);
			return 0;
		}
		++it;
	}
...
void	ID::Control::SetWindow(ID::Window* w)
{
	this->_window = w;
}
...
void	ID::Control::SetParent(ID::Drawable* p)
{
	...
	if (p == NULL && this->GetParent() != NULL)
	{
		w = this->GetParent()->GetRealWindow();
		if (w != NULL)
		{
...
ID::Window*	ID::Control::GetWindow(void) const
{
	return this->_window;
}
