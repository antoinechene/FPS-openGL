...
int		ID::Menu::ActionEvent(void)
{
	if (this->_cbActionEvent != NULL)
	{
		this->_cbActionEvent(this, NULL, this->__cbActionEventData);
		return 1;
	}
	return 0;
}

void		ID::Menu::SetCbActionEvent(ID::cbEvent cb, void* data)
{
	this->_cbActionEvent = cb;
	this->__cbActionEventData = data;
}

ID::cbEvent	ID::Menu::GetCbActionEvent(void)
{
	return this->_cbActionEvent;
}
...
int	ID::Menu::_BlitToWindow(void)
{
	ID::Window*	win;
	...
	if (win != NULL)
	{
		if (this->_surface->BlitSurface(this->_visibleRect,
						...
						this->_x, this->_y) == 0)
		{
			PRINT_DBG("Menu BlitToWindow X=" << this->_x << "	Y=" << this->_y << "\n");
			return 0;
		}
	}
	return -1;
