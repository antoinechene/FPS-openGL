...
Unit_Model::Unit_Model(char** tab)
{
	...
	this->_stateMachineID = atoi(tab[18]);
	max = atoi(tab[19]);
	i = 0;
	while (i < max)
	{
		script = sm->GetModel((SCRIPT_ID)atoi(tab[20 + i]));
		if (script == NULL)
			throw 1000;
...
uint32_t	Unit_Model::GetPopulation()
{
...
uint8_t	Unit_Model::GetStateMachineID()
{
